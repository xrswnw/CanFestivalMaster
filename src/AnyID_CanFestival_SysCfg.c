...
void Sys_Init(void)
{
    ...
    Sys_Delayms(500);                       //从机先启动
    setNodeId(& masterNormal_Data, 0x00);
   //setState(&masterNormal_Data, Initialisation);					//设备上电后初始状态
    setState(&masterNormal_Data, Pre_operational);
    setState(&masterNormal_Data, Operational);
...
*/
	...
	{	
        ...
        //canDispatch(&masterNormal_Data, &canRcvFrame);
	}
}
