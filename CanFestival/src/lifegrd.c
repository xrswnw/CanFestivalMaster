...
void _heartbeatError(CO_Data* d, UNS8 heartbeatID){}                //从站心跳丢失错误回调函数，代谢
void _post_SlaveBootup(CO_Data* d, UNS8 SlaveID)                    //收到从站上电报文让其启动
{
	masterSendNMTstateChange (d, SlaveID, NMT_Start_Node);
	d->NMTable[SlaveID] = Operational;
}
void _post_SlaveStateChange(CO_Data* d, UNS8 nodeId, e_nodeState newNodeState){}    //从站工作状态自己异常变更，可再次恢复或者报警
